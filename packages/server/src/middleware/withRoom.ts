...
export function createWithRoom(io: TypedServer) {
  return function withRoom<T = void>(
    ...
    return function (this: TypedSocket, data: T) {
      ...
      if (!mapping) {
        socket.emit(EVENTS.ROOM_ERROR, { code: 'NOT_IN_ROOM', message: '你不在任何房间中' })
        return
      }
      ...
      if (!user) {
        ...
        socket.emit(EVENTS.ROOM_ERROR, { code: 'NOT_IN_ROOM', message: '你不在该房间中' })
        return
      }
      ...
      Promise.resolve(handler(ctx, data)).catch((err) => {
        ...
        socket.emit(EVENTS.ROOM_ERROR, { code: 'INTERNAL', message: '服务器内部错误' })
      })
    }
  }
}
